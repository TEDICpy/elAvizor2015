...
?>
		...
		jQuery(window).load(function() {
			...
			var options = {
				...
				maxExtent: new OpenLayers.Bounds(v_bounds).transform(proj_4326, proj_900913),
				restrictedExtent: new OpenLayers.Bounds(v_bounds).transform(proj_4326, proj_900913),
			...
			// GeoLocalizacion.
			$('.btn_geoLocalizacion').on('click', function () {
				geoLocalizacion();
			});
			...
			*/
			...
			if(field_type != "file"){
				...
				var htmlString = '<div class="uneditable-input span3">' +
								  ...
								  '</span>' +
								  /*'<a href="#" class="btn fileupload-exists btn-danger" data-dismiss="fileupload">Borrar</a>' + */
								  /*"<a href=\"#\" class=\"btn btn-primary\" "+" onClick=\"addFormField('" + div + "','" + field + "','" + hidden_id + "','" + field_type + "'); return false;\">"+"<i class='icon-plus-sign'></i></a>" +*/
			...
			$.post("<?php echo url::site() . 'reports/geocode/' ?>", { address: address },
				function(data){
					if (data.status == 'success'){
						...
						var alertMessage = address + ' <?php echo Kohana::lang('ui_main.alert_message_report'); ?>';
						alert(alertMessage)
					}
			...
			if (navigator.geolocation) {
				navigator.geolocation.getCurrentPosition(
				  function coordenadas(position){
					  var v_longitud = position.coords.longitude;
					  var v_latitud = position.coords.latitude;

					  geocodeNominatim(v_longitud, v_latitud);
				});
			}else{
				 alert('Oops! Tu navegador no soporta geolocalización. Bájate Mozilla Firefox o Google Chrome, que es gratis!');

			}
		}

		function geocodeNominatim(p_longitude, p_latitude){
			$('#geo_loading').html('<img src="<?php echo url::file_loc('img')."media/img/loading_g.gif"; ?>">');
			$.post("<?php echo url::site() . 'reports/geocodeNominatim/' ?>", {
				...
				function(data){
					if (data.status == 'success'){
						// Clear the map first
  					    ...
  					    point = new OpenLayers.Geometry.Point(p_longitude, p_latitude);
  					    OpenLayers.Projection.transform(point, proj_4326,proj_900913);
  					    ...
  					    // create a new lat/lon object
  					    myPoint = new OpenLayers.LonLat(p_longitude, p_latitude);
  					    myPoint.transform(proj_4326, map.getProjectionObject());
						...
						// Update form values
						// $("#country_name").val(data.country);
						$("#latitude").val(p_latitude);
						$("#longitude").val(p_longitude);
						$("#location_name").val(data.location_name);
					} else {
						// Alert message to be displayed
						var alertMessage = "No pudo encontrar tu ubicaci\u00F3n";
						alert(alertMessage)
					}
					$('div#geo_loading').html('');
				}, "json");
			return false;
		}
