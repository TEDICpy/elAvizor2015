...
?>
	...
	$(document).ready(function() {
		...
		$("#filterReportByDate a.filter-button").click(function(){
			// Fecha from.
			var v_report_date_from_temp = $("#report_date_from").val();
			var v_formattedDateFrom = new Date(v_report_date_from_temp);
			var dia_from = v_formattedDateFrom.getDate();
			var m_from =  v_formattedDateFrom.getMonth();
			m_from += 1;  			// Los meses de JavaScript es de 0 al 11.
			var y_from = v_formattedDateFrom.getFullYear();
			var v_report_date_from = dia_from + "/" + m_from + "/" + y_from;

			// Fecha to.
			var v_report_date_to_temp = $("#report_date_to").val();
			var v_formattedDateTo = new Date(v_report_date_to_temp);
			var dia_to = v_formattedDateTo.getDate();
			var m_to =  v_formattedDateTo.getMonth();
			m_to += 1;  			// Los meses de JavaScript es de 0 al 11.
			var y_to = v_formattedDateTo.getFullYear();
			var v_report_date_to = dia_to + "/" + m_to + "/" + y_to;

			// Change the text
			//$(".time-period").text($("#report_date_from").val()+" to "+$("#report_date_to").val());
			$(".time-period").text(v_report_date_from + " a " + v_report_date_to);
